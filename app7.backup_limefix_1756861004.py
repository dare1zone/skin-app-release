...
def send_email_smtp(subject, body_text, to_email, from_email, smtp_host, smtp_port="465", smtp_user=None, smtp_pass=None, attachment=None, attachment_name=None, attachment_mime=None):
    import smtplib
    ...
    if attachment is not None:
        ...
        if isinstance(attachment_mime, str) and '/' in attachment_mime:
    ...
    with smtplib.SMTP_SSL(smtp_host, int(smtp_port)) as srv:
        if smtp_user:
            srv.login(smtp_user, smtp_pass or "")
        srv.send_message(msg)
    return True

def send_email_if_configured(subject, body_text, attachment_bytes=None, attachment_name=None, attachment_mime=None):
    import os
    ...
    to_email = os.getenv("SMTP_TO", os.getenv("ADMIN_EMAIL", user or ""))
    if host and from_email and to_email:
        try:
            return send_email_smtp(subject, body_text, to_email, from_email, host, port, user, pw, attachment=attachment_bytes, attachment_name=attachment_name, attachment_mime=attachment_mime)
...
def _build_grad_model(m):
    base = None
    for name in ["mobilenetv2_1.00_224","MobilenetV2","mobilenetv2_1.00_192","mobilenetv2_1.00_160"]:
        try:
            base = m.get_layer(name)
            break
        except Exception:
            pass
    if base is None:
        for L in m.layers:
            if hasattr(L, "get_layer") and hasattr(L, "layers") and len(getattr(L,"layers",[])) >= 20:
                try:
                    L.get_layer("block_13_expand_relu")
                    base = L
                    break
                except Exception:
                    pass
    feat_t = None
    if base is not None:
        for tname in ["block_13_expand_relu","block_7_expand","block_6_expand_relu","out_relu"]:
            try:
                feat_t = base.get_layer(tname).output
                break
            except Exception:
                pass
    if feat_t is None:
        for L in m.layers[::-1]:
            try:
                out = L.output
                if len(out.shape) == 4 and int(out.shape[1]) >= 7:
                    feat_t = out
                    break
            except Exception:
                pass
    ...
    return tf.keras.Model(inputs=m.input, outputs=[feat_t, m.output])
...
def load_ntd_artifacts():
    ...
    # pick a higher-res feature map for sharper CAM
    feat = None
...
def gradcam_uint8(img_tensor, grad_model, class_index, out_size, focus_pct=85.0, blur_sigma=0.8):
    ...
    # resize to output size first so thresholds are applied in display scale
    ...
    try:
        ...
        pct = 85.0
    pct = max(50.0, min(99.0, pct))  # clamp 50–99
    t = np.percentile(cam, pct)
    cam = np.where(cam >= t, cam, 0.0)
...
if nav == "Upload & Predict":
    ...
    if up is not None:
        ...
        if mode_key == "ntd":
            ...
            with thr_col1:
                min_conf = st.slider("Min confidence to accept (NTD)", 0.0, 1.0, 0.50, 0.01, key="ntd_min_conf")
            with thr_col2:
                min_margin = st.slider("Min margin top–2", 0.0, 0.5, 0.20, 0.01, key="ntd_min_margin")
            ...
            if conf < float(min_conf) or margin < float(min_margin):
                st.info(f"Inconclusive (decision gate) · conf={conf:.2f} thr>={float(min_conf):.2f} · margin={margin:.2f} thr>={float(min_margin):.2f}")
                st.stop()
    ...
    with colL:
        ...
        focus = st.slider("CAM focus (percentile, higher = tighter)", 50, 99, 90, 1, key="focus_cam")
        blur  = st.slider("CAM denoise (sigma)", 0.0, 2.0, 0.8, 0.05, key="blur_cam")
    ...
    with colR:
        if up is not None:
            ...
            if mode_key == "ntd":
                heat = gradcam_uint8(x, grad_model, t_idx, IMG_SIZE, focus, blur)
                blend = overlay_heat(rgb224, heat, alpha)
    ...
    if up is not None:
        try:
            ...
            base, _ = preprocess_pil(open_uploaded(up), tone, preprocess_fn)
            base_arr = base.astype("uint8")

            def classifier_fn(imgs):
                arrs = []
                for a in imgs:
                    a_uint8 = (a*255 if getattr(a, "max", lambda:1.0)() <= 1.0 else a).astype("uint8")
            ...
            st.image((outlined*255).astype("uint8"), caption=f"LIME — {label_to_explain}", width=360, use_container_width=False)

        except Exception as e:
            st.error(f"LIME error: {e}. Make sure 'lime' and 'scikit-image' are installed.")
    ...
    with st.form("findcare"):
        ...
        with col1:
            ...
            specialty = st.selectbox("Specialty", ["Dermatologist", "Infectious disease", "Tropical medicine", "Primary care clinic", "Skin clinic"], index=0, key="care_spec")
        with col2:
    ...
    with st.form("msg_doc"):
        ...
        if sent:
            if not ok or not contact.strip():
                ...
                import time, os, json, base64, smtplib, ssl
                from email.message import EmailMessage
                ...
                def extract_emails(text):
                    import re
                    return [m.group(0) for m in re.finditer(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}", text or "")]

                try:
                    if smtp_host and smtp_user and smtp_pass:
                        recipients = extract_emails(contact) or extract_emails(smtp_to_default)
                        if not recipients:
                            raise RuntimeError("No valid recipient email(s).")

                        email = EmailMessage()
                        email["Subject"] = subj or "Skin concern"
                        email["From"] = smtp_from
                        email["To"] = ", ".join(recipients)
                        email.set_content(f"""Dear {name or 'Patient'},
...
- Skin Lesion Assistant
                        if up2 is not None:
                            try:
                                ...
                                email.add_attachment(up2.getvalue(), maintype=maintype, subtype=subtype, filename=up2.name)
                            except Exception:
                                pass
                        ...
                        st.success("Email sent to: " + ", ".join(recipients))
                    else:
                        st.info("Email not sent: SMTP_* env vars not set. Logged locally only.")
                except Exception as e:
    ...
    with st.form("msg_doc"):
        ...
        with c2:
            subj = st.text_input("Subject", key="msg_subj")
            up2 = st.file_uploader("Attach an image (optional)", type=["jpg","jpeg","png","heic","heif"], key="msg_up")
        # NEW: optional override for recipients
        to_override = st.text_input("Recipient email(s) (comma-separated, optional)", key="msg_to_override", placeholder="doctor1@clinic.com, nurse@hospital.org")
        msg = st.text_area("Message", height=120, key="msg_text")
        ok = st.checkbox("I consent to be contacted about this message.", key="msg_ok")
        sent = st.form_submit_button("Send message")
        if sent:
            if not ok or not contact.strip():
                ...
                def extract_emails(text):
                    ...
                    emails = [e for _, e in pairs if "@" in e and "." in e.split("@")[-1]]
                ...
                # Priority: override field -> contact (if email) -> default inbox
                recipients = extract_emails(to_override)
                if not recipients:
                    recipients = extract_emails(contact)
                if not recipients:
                    recipients = [smtp_to_default]
...
email["Subject"] = subj or "Skin concern"
email["From"] = smtp_from
# send to any valid addresses the user typed, else to default fallback
recipients = extract_emails(contact) or extract_emails(smtp_to_default)
email["To"] = ", ".join(recipients) if recipients else smtp_to_default
email.set_content(f"""Dear {name or 'Patient'},
...
- Skin Lesion Assistant
                        if up2 is not None:
                            ...
                            if up2.type and "/" in up2.type:
                                maintype, subtype = up2.type.split("/",1)
                        ...
                        st.warning(f"Saved locally, but email failed: {e}")
                else:
                    st.info("Saved locally. Set SMTP_* env vars to enable email sending.")
