# === NTD model loader (ROI-trained) ===
@st.cache_resource
def get_ntd_model():
    import os, json, tensorflow as tf
    mpath = "models/ntd/ntd_mbv2_roi.keras"
    if not os.path.exists(mpath):
        st.error("NTD model file not found at models/ntd/ntd_mbv2_roi.keras")
        st.stop()
    ...
    # thresholds from Colab tuning
    per_class_thr = [0.55, 0.55, 0.593]  # [buruli, leish, leprosy]
    top2_margin   = 0.172
...
def preprocess_pil(pil_im, tone_mode, preprocess_fn):
    from roi_utils import border_crop, remove_edge_text2, lesion_bbox
    im = ImageOps.exif_transpose(pil_im).convert("RGB")
    base = np.array(im)
    base = tone_calibrate(base, tone_mode)
    base = border_crop(base, pct=0.06)
    base = remove_edge_text2(base, edge_pct=0.28)
...
if nav == "Upload & Predict":
    ...
    if up is not None:
        ...
        from roi_utils import lesion_bbox
        bbox = lesion_bbox(rgb224)
        if bbox is not None:
            import cv2
            y0,x0,y1,x1 = bbox
            _prev = rgb224.copy()
            cv2.rectangle(_prev, (x0,y0), (x1,y1), (14,165,164), 2)
            st.image(_prev, caption="Lesion ROI preview", width=320, use_container_width=False)

        if mode_key == "ntd":
            import cv2, numpy as np
        # use the specialized NTD ROI model
        model, ntd_labels, ntd_thr, ntd_margin = get_ntd_model()
        
            p = model.predict(x, verbose=0)[0]
            from guardrails import should_abstain_v2
            abstain, reason = should_abstain_v2(p, None, rgb224)
            if abstain:
                st.info(f"Inconclusive for NTD ({reason}).")
                st.stop()
            ...
            ycrcb = cv2.cvtColor(rgb224, cv2.COLOR_RGB2YCrCb)
            skin = cv2.inRange(ycrcb, (0,133,77), (255,173,127))
            skin_pct = float((skin>0).mean())
            if (conf < 0.70 or margin < 0.20) and ((lap_var < 30.0 and edge_ratio < 0.010) or skin_pct < 0.05):
                st.info("Inconclusive for NTD (likely normal skin/background or weak signal).")
                st.stop()
