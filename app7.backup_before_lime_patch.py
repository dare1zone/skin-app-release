...
if nav == "Upload & Predict":
    ...
    if up is not None:
        ...
        if mode_key == "ntd":
            ...
            if conf < 0.50 or margin < 0.25:
                st.info(f"Inconclusive (decision gate) · conf={conf:.2f} thr>=0.50 · margin={margin:.2f} thr>=0.25")
                st.stop()
        ...
        try:
            ...
            def classifier_fn(imgs):
                ...
                if mode_key == "ntd":
                    ...
                    return probs
            segments = slic(base_arr, n_segments=150, compactness=18, sigma=0, start_label=1)
