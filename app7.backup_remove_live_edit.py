...
if nav == "Upload & Predict":
    ...
    live_text = st.text_input("Live-edit proof: type to change this line", value=st.session_state.get("live_text","I built this app and can modify it live."), key="live_text")
    st.markdown(f"<div class='card small'>{live_text}</div>", unsafe_allow_html=True)
            ...
            if not ok or not contact.strip():
                ...
                try:
                    ...
                    if host and user and password and to_addr:
                        ...
                        msg_out["Subject"] = subj or "Skin concern"
                        msg_out["From"] = user
                        msg_out["To"] = to_addr
                        body = f"Name: {name or ''}\nContact: {contact or ''}\nCity: {city or ''}\n\n{msg or ''}"
                        msg_out.set_content(body)
                        if up2 is not None:
                            msg_out.add_attachment(up2.getvalue(), maintype="application", subtype="octet-stream", filename=up2.name)
                        with smtplib.SMTP(host, port, timeout=15) as s_smtp:
                            s_smtp.starttls()
                            s_smtp.login(user, password)
                            s_smtp.send_message(msg_out)
                        smtp_ok = True
                except Exception as e:
