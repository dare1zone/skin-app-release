...
if nav == "Upload & Predict":
    ...
    with colL:
        ...
        focus = st.slider("CAM focus (percentile, higher = tighter)", 50, 99, 90, 1, key="focus_main")
        blur  = st.slider("CAM denoise (sigma)", 0.0, 2.0, 0.8, 0.05, key="blur_main")
    with colR:
...
import cv2, numpy as np
            ...
            if conf < min_conf or margin < min_margin:
                st.info(f"Inconclusive for NTD (conf={conf:.2f}, margin={margin:.2f}). Try a closer, sharp dermatoscopic image.")
                st.stop()
            conf_html, stab_html, order = conf_stability(p)
            pred_idx = int(order[0])
            heat = gradcam_uint8(x, grad_model, pred_idx, IMG_SIZE, focus, blur)
            blend = overlay_heat(rgb224, heat, alpha)
            st.markdown(f"{conf_html} {stab_html} <span class='small'>Top={float(np.max(p))*100:.1f}%</span>", unsafe_allow_html=True)
    ...
    num_feat = st.slider("Number of regions", 4, 20, 9, 1, key="lime_num")

    # extra controls
    cA, cB, cC = st.columns(3)
    with cA:
        n_segments = st.slider("Superpixels (segments)", 80, 300, 180, 10, key="lime_segs")
    with cB:
        compactness = st.slider("Compactness", 5, 30, 20, 1, key="lime_comp")
    with cC:
        mode = st.radio("Evidence", ["positive","negative","both"], horizontal=True, key="lime_mode")
    ...
    if up is not None:
        try:
            ...
            explainer = lime_image.LimeImageExplainer(random_state=0, kernel_width=0.25)
            ...
            pos_only = (mode == "positive")
            neg_only = (mode == "negative")
            ...
            overlay[mask == 0] = (overlay[mask == 0] * 0.25 + 255 * 0.75).astype(np.uint8)
            outlined = mark_boundaries(overlay/255.0, mask, color=(1,1,0), mode="thick")
            st.image((outlined*255).astype(np.uint8), caption=f"LIME — {label_to_explain}", width=360, use_container_width=False)

                        with st.expander("Show superpixels (debug)"):
                from skimage.segmentation import mark_boundaries
                st.image((mark_boundaries(base_arr/255.0, segments, color=(0,1,1))*255).astype(np.uint8), caption="Superpixels", width=320, use_container_width=False)
            lime_buf = io.BytesIO()
            Image.fromarray((outlined*255).astype(np.uint8)).save(lime_buf, format="PNG"); lime_buf.seek(0)
            st.download_button("Download LIME (PNG)", data=lime_buf, file_name="lime_overlay.png", mime="image/png")
contribs = sorted(exp.local_exp[lab], key=lambda t: -abs(t[1]))[:min(10, int(num_feat))]
            st.dataframe(
                {"region": [int(i) for i, _ in contribs],
                 "weight": [round(float(w), 4) for _, w in contribs]},
