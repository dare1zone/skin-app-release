...
def load_general_artifacts():
    ...
    try:
        ...
        def pp_f(x):
            return (x/127.5)-1.0
        dummy_inp = tf.keras.Input(shape=IMG_SIZE+(3,))
        dummy_conv = tf.keras.layers.Lambda(lambda z: tf.image.resize(z, (7,7)))(dummy_inp)
        dummy_probs = tf.keras.layers.GlobalAveragePooling2D()(dummy_conv)
        dummy_probs = tf.keras.layers.Dense(len(GENERAL_LABELS), activation="softmax")(dummy_probs)
        cam_model = tf.keras.Model(dummy_inp, [dummy_conv, dummy_probs])
        return cam_model, GENERAL_LABELS, pp_f, "general"

def _find_backbone_and_feat_v2(m):
    inner = None
    for L in m.layers:
        if hasattr(L, "get_layer") and hasattr(L, "layers") and len(L.layers) > 20:
            inner = L
            break
    if inner is None:
        try:
            feat = m.get_layer("out_relu").output
            return m, feat
        except Exception:
            pass
        last4 = None
        ...
        if last4 is not None:
            return m, last4.output
        raise RuntimeError("Backbone not found in model.")
    for name in ["block_13_expand_relu","out_relu","block_6_expand_relu","block_7_expand"]:
        try:
            feat = inner.get_layer(name).output
            return inner, feat
        except Exception:
            continue
    last4 = None
    for L in inner.layers[::-1]:
        shp = getattr(getattr(L, "output", None), "shape", None)
        if hasattr(shp, "__len__") and len(shp)==4:
            last4 = L
            break
    if last4 is None:
        raise RuntimeError("No 4D conv layer found.")
    return inner, last4.output
...
def load_ntd_artifacts():
    ...
    inner, feat_t = _find_backbone_and_feat_v2(m)
    grad_model = tf.keras.Model(inputs=m.input, outputs=[feat_t, m.output])
...
if nav == "Upload & Predict":
    ...
    if up is not None:
        ...
        if mode_key == "ntd":
            ...
            if lap_var < 35.0 and edge_ratio < 0.020:
                st.info(f"Inconclusive (background gate) · lap={lap_var:.1f} thr>35.0 · edges={edge_ratio:.3f} thr>0.020")
                st.stop()
            ...
            if conf < GATE_CONF or margin < GATE_MARGIN:
                st.info(f"Inconclusive (decision gate) · conf={conf:.2f} thr>={GATE_CONF:.2f} · margin={margin:.2f} thr>={GATE_MARGIN:.2f}")
                st.stop()
    ...
    if submit:
        ...
        if not loc:
            loc = "near me"
        base_q = (q + " near " + loc) if loc != "near me" else (q + " near me")
        gq = base_q.replace(" ", "+")
...
def _find_backbone_and_feat_v2(m):
    def all_layers(root):
        ...
        while stack:
            ...
            seen.add(id(L))
            yield L
            if hasattr(L,'layers') and isinstance(getattr(L,'layers'), (list,tuple)):
                stack.extend(L.layers)
    cand=None
    for L in reversed(list(all_layers(m))):
        out=getattr(L,'output',None)
        try:
            shp=tuple(getattr(out,'shape',()))
        except Exception:
            shp=None
    ...
    if cand is None:
        try:
            cand=m.get_layer('out_relu').output
        except Exception:
            raise RuntimeError('Backbone not found in model.')
    return m, cand
