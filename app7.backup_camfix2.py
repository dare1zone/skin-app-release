...
def load_ntd_artifacts():
    ...
    grad_model = _build_grad_model(m)
...
def gradcam_uint8(img_tensor, grad_model, class_index, out_size):
    ...
    with tf.GradientTape() as tape:
        feat, preds = grad_model(img_tensor)
        cls = preds[:, int(class_index)]
    grads = tape.gradient(cls, feat)
    w = tf.reduce_mean(grads, axis=(1,2), keepdims=True)
    cam = tf.reduce_sum(w * feat, axis=-1)[0].numpy()
    cam = np.maximum(cam, 0)
    cam = cv2.resize(cam, out_size[::-1], interpolation=cv2.INTER_LINEAR)
    vmin = float(np.percentile(cam, 60.0))
    vmax = float(np.percentile(cam, 99.0))
    cam = (cam - vmin) / (vmax - vmin + 1e-6)
    cam = np.clip(cam, 0.0, 1.0)
    thr = float(np.percentile(cam, 72.5))
    cam = cam * (cam >= thr)
