...
if nav == "Upload & Predict":
    ...
    if up is not None:
        ...
        if mode_key == "ntd":
            ...
            if lap_var < 15.0 and edge_ratio < 0.010:
            ...
            if conf < 0.60 or margin < 0.12:
                st.info(f"Inconclusive (decision gate) · conf={conf:.2f} thr>=0.60 · margin={margin:.2f} thr>=0.12")
                st.stop()
            conf_html, stab_html, order = conf_stability(p)
            pred_idx = int(order[0])
            heat = scorecam_uint8(x, grad_model, pred_idx, IMG_SIZE, k=24)
...
focus_cam = st.slider("CAM focus (percentile, higher = tighter)", 70, 95, 85, 1, key="focus_cam")
blur_cam  = st.slider("CAM denoise (sigma)", 0.0, 1.2, 0.8, 0.1, key="blur_cam")
    target = st.selectbox("Class to visualize", CLASS_NAMES, key="cam_target")
    if up is not None:
        rgb224, x = preprocess_pil(open_uploaded(up), tone, preprocess_fn)
        t_idx = CLASS_NAMES.index(target)
        if mode_key == "ntd":
            heat = gradcam_uint8(x, grad_model, t_idx, IMG_SIZE, focus_cam, blur_cam)
            blend = overlay_heat(rgb224, heat, alpha)
