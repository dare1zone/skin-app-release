...
def gradcam_uint8(img_tensor, grad_model, class_index, out_size):
    ...
    A = conv_out[0]
    G = tf.nn.relu(grads[0])
    num = tf.reduce_sum(G * A, axis=(0, 1))
    den = tf.reduce_sum(A, axis=(0, 1)) + 1e-6
    w = num / den

    cam = tf.reduce_sum(A * w, axis=-1)
    cam = tf.nn.relu(cam).numpy()

    import numpy as np, cv2
    lo = float(np.percentile(cam, 5.0))
    hi = float(np.percentile(cam, 99.0))
