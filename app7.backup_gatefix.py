    ...
    if up is not None:
        ...
        if mode_key == "ntd":
            ...
            margin = float(conf - (p[order_tmp[1]] if len(p) > 1 else 0.0))
                        pred_idx = int(order_tmp[0])
            thr_bu = 0.70 if CLASS_NAMES[pred_idx].lower().startswith("buruli") else 0.55
            margin_cut_pair = 0.20 if (CLASS_NAMES[pred_idx].lower().startswith("buruli") and CLASS_NAMES[order_tmp[1]].lower().startswith("leish")) else 0.12
if conf < thr_bu or margin < 0.12:
                st.info(f"Inconclusive (decision gate) · conf={conf:.2f} thr>={thr_bu:.2f} · margin={margin:.2f} thr>={margin_cut_pair:.2f}")
                st.stop()
