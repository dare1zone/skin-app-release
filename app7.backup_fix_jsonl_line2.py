...
if nav == "Upload & Predict":
    ...
    override = st.session_state.get("helpbot_override", "").strip()
    ...
    with st.expander("Live edit proof (exam)"):
        col1, col2 = st.columns([2,1])
        with col1:
            st.text_area("Override answer for 'What is this tool for?'", value=override, key="helpbot_override")
        with col2:
            st.color_picker("Accent color", st.session_state.get("accent_color", "#10B981"), key="accent_color")
        st.markdown(f"<div style='height:6px;background:{st.session_state.get('accent_color','#10B981')};border-radius:4px;margin:8px 0 0 0'></div>", unsafe_allow_html=True)
            ...
            if not ok or not contact.strip():
                ...
                if up2 is not None:
                    try:
                        ...
                        attach_name = up2.name
                        attach_mime = up2.type
                        attach_bytes = up2.getvalue()
                        rec["image_b64"] = base64.b64encode(attach_bytes).decode("ascii")
                    except Exception:
                        pass
                try:
...
Message:
{msg or '-'}"
                send_res = send_email_if_configured(subject, body, attachment_bytes=attach_bytes, attachment_name=attach_name, attachment_mime=attach_mime)
                if send_res is True:
                    st.success("Email sent to clinic inbox.")
                elif send_res == "EMAIL_NOT_CONFIGURED":
                    st.info("Email not configured. Saved locally to messages_outbox.jsonl.")
                else:
                    st.warning(f"Email error: {send_res}. Saved locally to messages_outbox.jsonl." if json_ok else f"Email error: {send_res}.")
                draft = f"""To: {{contact}}
Subject: {{subject}}
