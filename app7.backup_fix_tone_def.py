...
def tone_calibrate(def tone_calibrate(arr_uint8, mode):
    if mode == "none":
        return arr_uint8
...
def gradcam_uint8(img_tensor, grad_model, class_index, out_size):
    ...
    # light denoise + keep only strongest activations
    import cv2, numpy as np
    cam = cv2.GaussianBlur(cam, (0, 0), 1.0)
    keep = np.percentile(cam, 75.0)  # keep top 25%
    cam = np.clip((cam - keep) / (1e-6 + 1 - keep), 0, 1)
...
def scorecam_uint8(def scorecam_uint8(x_pre, cam_model, class_index, out_size, k=16):
