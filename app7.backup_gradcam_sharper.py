...
def gradcam_uint8(img_tensor, grad_model, class_index, out_size):
    ...
    for _ in range(n):
        x = img_tensor
        if noise > 0:
            eps = tf.random.normal(tf.shape(x), stddev=noise)
            x = tf.clip_by_value(x + eps, -1.0, 1.0)
        with tf.GradientTape() as tape:
            conv_out, preds = grad_model(x)
            loss = tf.math.log(preds[:, int(class_index)] + 1e-6)
    ...
    lo, hi = np.quantile(cam, 0.25), np.quantile(cam, 0.995)
